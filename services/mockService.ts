...
const DEFAULT_SETTINGS: AppSettings = {
  ...
  navigationAdReward: 5, // Default 5 coins for navigation ads
...
export const loginUser = (identifier: string, passwordOrMobile: string, isMobileLogin: boolean): User | null => {
  ...
  if (user) {
    ...
    if (user.deviceId !== currentDeviceId) {
        // In strict mode, we might block. For demo, we just warn or allow if "Auto Login" was intended.
    }
...
export const getSystemStats = () => {
    ...
    return {
        ...
        totalCoinsGiven: users.reduce((acc, u) => acc + u.coins, 0), // Simplified: Current balance as proxy or calculate from transactions
